  ...
  return (
    <div className="space-y-8 animate-fade-in">
          ...
          return (
            <div
              ...
              className={`group relative cursor-pointer rounded-2xl p-4 md:p-5 transition-all duration-300 border ${
                isSelected
                  ? 'border-[#ff6a00] bg-gradient-to-b from-[#ff6a00]/15 to-[#131b2e] shadow-[0_0_30px_-8px_rgba(255,106,0,0.5)] scale-[1.02]'
                  : 'border-white/10 bg-[#111726]/60 hover:border-white/20 hover:bg-[#111726]/90'
              }`}
            >
              {/* Badge */}
              <div className="flex items-center justify-between mb-3">
                <div className={`flex h-10 w-10 items-center justify-center rounded-xl transition-transform group-hover:scale-110 ${
                  isSelected ? 'bg-[#ff6a00] text-white shadow-lg shadow-[#ff6a00]/40' : 'bg-white/5 text-slate-300'
                }`}>
              ...
              <p className="text-xs text-[#ff7d1a] font-medium mt-0.5">
                {preset.dimensions}
              </p>
      ...
      {inputs.roomType === 'custom' && (
        <div className="rounded-3xl border border-[#ff6a00]/30 bg-[#131b2e]/80 p-5 md:p-6 backdrop-blur-md">
          ...
          {inputs.useManualM2 ? (
            <div className="mt-4 max-w-sm">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-2xl border border-white/15 bg-white/5 px-4 py-3 text-lg font-bold text-white focus:border-[#ff6a00] focus:outline-none"
                  placeholder="Ej: 45"
                />
            ...
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div>
                <label className="block text-xs text-slate-400 mb-1">Largo (m)</label>
                <input
                  type="number"
                  step="0.1"
                  value={inputs.length || 4}
                  onChange={(e) => onChangeInputs({ length: parseFloat(e.target.value) || 0 })}
                  className="w-full rounded-xl border border-white/15 bg-white/5 px-3 py-2 text-sm font-semibold text-white focus:border-[#ff6a00] focus:outline-none"
                />
              </div>
              <div>
                <label className="block text-xs text-slate-400 mb-1">Ancho (m)</label>
                <input
                  type="number"
                  step="0.1"
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1">Alto pared (m)</label>
                <input
                  type="number"
                  step="0.1"
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1">Descontar aberturas (m²)</label>
                <input
                  type="number"
                  step="0.5"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <button
          ...
          className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-left hover:border-[#ff6a00]/40 hover:bg-[#ff6a00]/5 transition-all group"
        >
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/5 text-[#ff7d1a] group-hover:bg-[#ff6a00] group-hover:text-white transition-colors">
              <Ruler className="h-5 w-5" />
            </div>
            <div>
              <p className="text-xs font-bold text-white">¿No sabés cómo medir tus paredes?</p>
              <p className="text-[11px] text-slate-400">Abrir instructivo interactivo con resta de puertas y ventanas</p>
            </div>
          </div>
          <ArrowRight className="h-4 w-4 text-slate-500 group-hover:text-[#ff6a00] transition-colors" />
        </button>
        ...
        >
          <div className="flex items-center gap-3">
            <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${
              inputs.paintCeiling ? 'bg-[#ff6a00] text-white' : 'bg-white/5 text-slate-400'
            }`}>
              <Sparkles className="h-5 w-5" />
            </div>
            <div>
              <p className="text-xs font-bold text-white">¿Pintar también el techo / cielorraso?</p>
              <p className="text-[11px] text-slate-400">Añade pintura látex especial antihongo mate</p>
            </div>
          </div>

          <div className={`h-6 w-11 rounded-full p-1 transition-colors ${
            inputs.paintCeiling ? 'bg-[#ff6a00]' : 'bg-white/10'
          }`}>
