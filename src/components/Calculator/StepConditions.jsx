...
export function StepConditions({ inputs, onChangeInputs, onPrevStep, onCalculate }) {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
          ...
          return (
            <div
              ...
              className={`group flex items-start gap-4 p-4 md:p-5 rounded-2xl border cursor-pointer transition-all duration-200 ${
                active
                  ? 'border-[#ff6a00] bg-gradient-to-r from-[#ff6a00]/15 to-[#131b2e] shadow-[0_0_25px_-5px_rgba(255,106,0,0.4)] scale-[1.01]'
            ...
            >
              <div className={`mt-0.5 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl transition-colors ${
                active ? 'bg-[#ff6a00] text-white shadow-md shadow-[#ff6a00]/40' : 'bg-white/5 text-slate-400'
              }`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className={`h-5 w-5 rounded-lg border flex items-center justify-center transition-all ${
                    active ? 'border-[#ff6a00] bg-[#ff6a00] text-white' : 'border-white/20 bg-white/5'
                  }`}>
                ...
                <div className="mt-2 flex items-center gap-2">
                  <span className="text-[10px] font-mono font-semibold text-[#ff7d1a] bg-[#ff6a00]/10 px-2 py-0.5 rounded border border-[#ff6a00]/20">
      ...
      <div className="rounded-3xl border border-white/15 bg-gradient-to-b from-[#131b2e] to-[#0d1322] p-6 shadow-xl">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-base font-bold text-white">¿Quién se encarga de comprar los materiales?</h3>
            <p className="text-xs text-slate-400">Elegí la modalidad para personalizar el desglose de tu presupuesto</p>
          </div>
        </div>
          ...
          <div
            ...
            className={`p-4 rounded-2xl border cursor-pointer transition-all ${
              inputs.materialsBuyer === 'client'
                ? 'border-yellow-400/80 bg-yellow-400/10 shadow-[0_0_25px_-5px_rgba(250,204,21,0.25)]'
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <div className={`h-4 w-4 rounded-full border flex items-center justify-center ${
                inputs.materialsBuyer === 'client' ? 'border-yellow-400 bg-yellow-400' : 'border-white/20'
              }`}>
                {inputs.materialsBuyer === 'client' && <div className="h-2 w-2 rounded-full bg-slate-900" />}
              </div>
            </div>
            <h4 className="text-sm font-bold text-white">Compro yo los materiales (Mercado Libre)</h4>
          ...
          <div
            ...
            className={`p-4 rounded-2xl border cursor-pointer transition-all ${
              inputs.materialsBuyer === 'painter'
                ? 'border-[#ff6a00] bg-[#ff6a00]/10 shadow-[0_0_25px_-5px_rgba(255,106,0,0.3)]'
                : 'border-white/10 bg-white/[0.03] hover:border-white/20'
            }`}
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <div className={`h-4 w-4 rounded-full border flex items-center justify-center ${
                inputs.materialsBuyer === 'painter' ? 'border-[#ff6a00] bg-[#ff6a00]' : 'border-white/20'
              }`}>
                {inputs.materialsBuyer === 'painter' && <div className="h-2 w-2 rounded-full bg-slate-900" />}
              </div>
            </div>
            <h4 className="text-sm font-bold text-white">El pintor se encarga de todo</h4>
            <p className="text-xs text-slate-300 mt-1 leading-relaxed">
      ...
      <div className="flex items-center justify-between pt-4">
        <button
          type="button"
          onClick={onPrevStep}
          className="flex items-center gap-2 rounded-2xl border border-white/15 px-6 py-3.5 text-sm font-semibold text-slate-300 hover:bg-white/5 transition-all"
        >
