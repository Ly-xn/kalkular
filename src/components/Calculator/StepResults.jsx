  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="text-center max-w-xl mx-auto">
        <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3.5 py-1 text-xs font-bold text-emerald-400 uppercase tracking-wider mb-2">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-2">
        <div className="flex items-center gap-1.5 rounded-xl border border-white/10 bg-white/5 p-1 text-xs">
          <span className="text-slate-400 px-2 font-medium">Moneda:</span>
          <button
            onClick={() => setCurrencyMode('ARS')}
            className={`px-3 py-1 rounded-lg font-bold transition-all ${
              currencyMode === 'ARS' ? 'bg-[#ff6a00] text-white shadow' : 'text-slate-300 hover:text-white'
            }`}
          >
            ARS ($)
          </button>
          <button
            ...
            className={`px-3 py-1 rounded-lg font-bold transition-all ${
              currencyMode === 'USD' ? 'bg-[#ff6a00] text-white shadow' : 'text-slate-300 hover:text-white'
            }`}
          >
            USD (Ref)
          </button>
      ...
      <div className="relative overflow-hidden rounded-3xl border border-white/15 bg-gradient-to-b from-[#131b2e] to-[#0d1322] p-6 md:p-8 shadow-2xl">
        
        {/* Glow de acento */}
        <div className="pointer-events-none absolute -top-24 right-0 h-72 w-72 rounded-full bg-[#ff6a00]/15 blur-3xl" />

        {/* Cifra Principal: Presupuesto Recomendado */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-center pb-8 border-b border-white/10">
          
          <div className="lg:col-span-2">
            <span className="text-xs font-bold uppercase tracking-wider text-[#ff7d1a]">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[11px] text-slate-400">Tiempo de ejecución</p>
                <p className="text-sm font-bold text-white">{labor.estimatedDays} días hábiles aprox.</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[11px] text-slate-400">Equipo sugerido</p>
                <p className="text-sm font-bold text-white">{labor.workforceAdvice}</p>
              </div>
            </div>
          </div>
        ...
        <div className="py-6 border-b border-white/10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-3.5">
              <span className="text-[11px] text-slate-400 block">Tarifa Mínima</span>
              <p className="text-lg font-bold text-slate-200 mt-0.5">{formatPrice(labor.min)}</p>
              <span className="text-[10px] text-slate-500">Pintor particular / sin garantía formal</span>
            </div>

            <div className="rounded-2xl border border-[#ff6a00]/40 bg-[#ff6a00]/10 p-3.5 shadow-[0_0_20px_-5px_rgba(255,106,0,0.3)]">
              <span className="text-[11px] font-bold text-[#ff7d1a] block">Recomendado / Promedio</span>
              <p className="text-xl font-black text-white mt-0.5">{formatPrice(labor.recommended)}</p>
              <span className="text-[10px] text-[#ffb480]">Excelente balance calidad / precio</span>
            </div>

            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-3.5">
              <span className="text-[11px] text-slate-400 block">Tarifa Máxima / Empresa</span>
              <p className="text-lg font-bold text-slate-200 mt-0.5">{formatPrice(labor.max)}</p>
        ...
        <div className="py-6 space-y-3">
          ...
          <div className="space-y-2 text-xs">
            <div className="flex justify-between items-center py-1.5 border-b border-white/5">
              ...
              <span className="font-semibold text-white">{formatPrice(labor.baseWallLabor)}</span>
            </div>
            ...
            {surfaces.ceilingM2 > 0 && (
              <div className="flex justify-between items-center py-1.5 border-b border-white/5">
                ...
                <span className="font-semibold text-white">{formatPrice(labor.baseCeilingLabor)}</span>
              </div>
            )}
            ...
            {labor.conditionSurchargesTotal > 0 && (
              <div className="flex justify-between items-center py-1.5 border-b border-white/5">
                ...
                <span className="font-semibold text-[#ff7d1a]">+{formatPrice(labor.conditionSurchargesTotal)}</span>
              </div>
            )}
            ...
            <div className="flex justify-between items-center pt-3 text-sm font-bold">
              <span className="text-white">Presupuesto Global Estimado (Mano de Obra + Materiales):</span>
        ...
        <div className="my-6 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-4 flex items-start gap-3.5 text-xs">
          <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-amber-400/20 text-amber-300 flex-shrink-0 mt-0.5">
        ...
        <div className="pt-6 border-t border-white/10 flex flex-col sm:flex-row gap-3">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 rounded-2xl bg-white/10 hover:bg-white/15 border border-white/15 py-3.5 text-xs md:text-sm font-bold text-white transition-all hover:scale-[1.02] active:scale-[0.98]"
          >
